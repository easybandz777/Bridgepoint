#!/usr/bin/env python3
"""
Build the Bridgepointe ↔ Stripe Onboarding PDF.

Same brand + ReportLab pipeline as the Crew Portal Owner's Guide. Content
sources are four markdown files in ./sections.
"""
...
OUTPUT = ROOT / "Bridgepointe-Stripe-Onboarding.pdf"

SECTIONS = [
    ("01-the-plan.md",         "I",   "The Plan",
        "Why we're moving off QuickBooks, and how Stripe replaces it."),
    ("02-stripe-signup.md",    "II",  "Signing Up at Stripe",
        "Step-by-step at stripe.com — what to click, what to gather, what to send."),
    ("03-day-to-day.md",       "III", "How You'll Use It",
        "The Monday-Friday operations runbook for you and Brenda."),
    ("04-costs-and-faq.md",    "IV",  "Costs, FAQ & Risk",
        "Real fee math, honest tradeoffs, and answers to the questions you'll have."),
]
...
S_BODY_LEFT = style("bodyL", fontName="Times-Roman", fontSize=10.5,
...
S_BULLET = style("bullet", fontName="Times-Roman", fontSize=10.5,
                 leading=15, textColor=INK, alignment=TA_LEFT,
                 leftIndent=18, bulletIndent=4, spaceAfter=4)
S_NUMBER = style("number", fontName="Times-Roman", fontSize=10.5,
                 leading=15, textColor=INK, alignment=TA_LEFT,
                 leftIndent=24, bulletIndent=6, spaceAfter=4)
        ...
        if re.match(r"^[-*]\s", stripped):
            while i < n and re.match(r"^[-*]\s", lines[i].strip()):
                ...
                while i < n and lines[i].startswith("  ") and lines[i].strip() != "" \
                    ...
                    txt += " " + lines[i].strip()
                    i += 1
                # Render checkbox-style "[ ]" prefixed lines as boxed bullets
                if txt.startswith("[ ] "):
                    txt = txt[4:]
...
def render_table(lines):
    ...
    t = Table(rendered, colWidths=col_w, hAlign="LEFT", repeatRows=1)
...
def main_template(canvas, doc):
    ...
    canvas.drawString(MARGIN_X + 1.05 * inch, PAGE_H - 0.55 * inch, "STRIPE ONBOARDING")
    canvas.setFillColor(INK_MUTE)
    canvas.drawRightString(PAGE_W - MARGIN_X, PAGE_H - 0.55 * inch, "Owner Briefing · 2026")
    canvas.setStrokeColor(GOLD)
    ...
    canvas.setFillColor(INK_MUTE)
    canvas.setFont("Helvetica", 8)
    canvas.drawString(MARGIN_X, 0.45 * inch, "Bridgepointe · Stripe Onboarding Briefing")
...
def build():
    doc = BaseDocTemplate(
        ...
        title="Bridgepointe — Stripe Onboarding Briefing",
        author="Bridgepointe",
        subject="Stripe onboarding briefing for the Bridgepointe owner",
    )
    ...
    flows.append(Paragraph("Stripe Onboarding", S_COVER_EYEBROW))
    flows.append(Spacer(1, 4))
    flows.append(Paragraph("Owner Briefing", S_COVER_TITLE))
    flows.append(Spacer(1, 18))
    flows.append(Paragraph(
        "Replacing QuickBooks for invoicing, payments,<br/>"
        "and bookkeeping — what to sign up for and why.",
        S_COVER_SUB))
    ...
    flows.append(Paragraph(
        "<i>This briefing is prepared as a single document so you can read it in "
        "one sitting, sign up at Stripe, and send back the API keys to your "
        "stepson. Estimated reading time: 25 minutes.</i>",
        S_LEAD))
    ...
    flows.append(Paragraph("BRIDGEPOINTE",
                            style("end_brand", fontName="Times-Bold",
    ...
    flows.append(Paragraph(
        f"Stripe Onboarding Briefing · {datetime.now().strftime('%B %Y')}",
        S_COVER_FOOT))

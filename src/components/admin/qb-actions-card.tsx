...
import { RefreshCw, PlugZap, AlertCircle, CheckCircle, Loader2, ExternalLink } from 'lucide-react';
    ...
    return (
        <div className="flex flex-col sm:flex-row sm:items-center gap-4 p-5 bg-black/20 border border-white/6 rounded-xl">
            ...
            <button
                ...
                className="h-10 px-4 rounded-xl bg-white/5 border border-white/10 text-white/85 text-xs font-semibold hover:text-white hover:bg-white/10 transition-colors flex items-center gap-2 disabled:opacity-40 disabled:cursor-not-allowed shrink-0"
            >
...
export function QbActionsCard({ disabled, onAfter }: QbActionsCardProps) {
    const [busy, setBusy] = useState<'sweep' | 'process' | 'refresh' | null>(null);
    const [results, setResults] = useState<{
        sweep: ActionResult | null;
        process: ActionResult | null;
        refresh: ActionResult | null;
    }>({ sweep: null, process: null, refresh: null });

    function setResult(key: 'sweep' | 'process' | 'refresh', value: ActionResult) {

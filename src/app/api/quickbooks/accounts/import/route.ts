...
/**
 * POST /api/quickbooks/accounts/import
 * Body: { actor?: string }
 *
 * Pulls every QB Account (paginated) and upserts into the CRM `accounts`
 * table. Per-account errors are counted in `failed` and do not abort the run.
...
export async function POST(req: Request) {
    ...
    try {
        ...
        const result = await importAllAccountsFromQb({ actor: body.actor ?? 'admin' });
